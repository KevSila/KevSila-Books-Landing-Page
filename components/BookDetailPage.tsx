...
const BookDetailPage: React.FC<{ book: Book }> = ({ book }) => {
  ...
  return (
    <main id="main-content">
      <section className={`book-detail-hero book-detail-${book.theme} px-5 pb-20 pt-32 md:px-8 md:pb-28 md:pt-40`}>
        <div className="mx-auto grid max-w-6xl items-center gap-14 lg:grid-cols-[.82fr_1.18fr]">
          ...
          <div>
            ...
            <p className="eyebrow mt-8">{book.eyebrow}</p>
            <h1 className="mt-4 font-display text-5xl font-black leading-none tracking-[-.045em] text-stone-950 md:text-7xl">{book.title}</h1>
            <p className="mt-4 font-sans text-sm font-bold uppercase tracking-[.15em] text-stone-500">{book.subtitle}</p>
            <p className="mt-8 max-w-2xl font-serif text-2xl font-semibold leading-9 text-stone-800">{book.shortPromise}</p>
            <p className="mt-5 max-w-2xl font-serif text-lg leading-8 text-stone-600">{book.description}</p>
            <div className="mt-8 flex flex-wrap gap-2">
              {book.tags.map((tag) => <span key={tag} className="benefit-chip">{tag}</span>)}
            </div>
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <a href="#buy" className="button-primary">Choose a format</a>
              <a href="#listen" className="button-secondary"><Headphones className="h-4 w-4" aria-hidden="true" /> Listen first</a>
            </div>
          </div>
        </div>
      </section>
